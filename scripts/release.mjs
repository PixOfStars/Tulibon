...
const keyPath = join(homedir(), '.tauri', 'ai-vision.key');
if (!existsSync(keyPath)) {
  console.error('❌ Signing key not found at ' + keyPath);
  console.error('   Generate it first: npx tauri signer generate -w ' + keyPath + ' -p ai-vision --ci');
  process.exit(1);
}
...
function findArtifact(ver) {
  const inst = join(nsisDir, `AI-Vision_${ver}_x64-setup.exe`);
  const sig = join(nsisDir, `AI-Vision_${ver}_x64-setup.exe.sig`);
...
try {
  execSilent(`gh release delete ${prevTag} --repo PixOfStars/AI-Vision --yes`, {
...
exec(`gh release create ${tag} "${artifacts.inst}" "${artifacts.sig}" --repo PixOfStars/AI-Vision --title "${tag}" --notes "${releaseNote}"`, {
  env: { ...process.env, GH_TOKEN: ghtoken },
});
...
function pushFile(path, content, commitMsg) {
  ...
  ghApi(`repos/PixOfStars/AI-Vision/contents/${path} --input "${tmpPayload}" -X PUT`);
...
const latestJson = {
  ...
  platforms: {
    "windows-x86_64": {
      ...
      url: `https://github.com/PixOfStars/AI-Vision/releases/download/${tag}/${installerName}`,
    },
  },
};
...
console.log(`   https://github.com/PixOfStars/AI-Vision/releases/tag/${tag}\n`);
