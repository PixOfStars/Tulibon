...
const REGISTRY_URL: &str =
    "https://github.com/PixOfStars/Tulibon/releases/latest/download/plugins.json";
...
// ── Read plugin file (replaces plugin:// custom protocol) ──
...
pub fn read_plugin_file(
    ...
    if !full_path.exists() {
        return Err(format!("找不到文件: {}", file_path));
    }
